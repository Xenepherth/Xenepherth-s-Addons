...
public abstract partial class UCE_Projectile : SkillEffect
{
    ...
    public UCE_ProjectileSkill projectileSkill;
    ...
    {
        GameObject go = Instantiate(projectileSkill.projectile.gameObject, caster.skills.effectMount.position, transform.rotation);
    ...
    {
        if (rotationSpeed > 0)
            transform.RotateAround(transform.position, Vector3.forward, (float)NetworkTime.time * rotationSpeed);
        ...
        if (lockDirection)
            ...
            if (data.sidekick)
            {
                ...
                projectileSkill.projectile.gameObject.transform.Rotate(new Vector3(0, fDegrees, 0));
            }
            else
            {
                projectileSkill.projectile.gameObject.transform.LookAt(target.collider.bounds.center);
            }
