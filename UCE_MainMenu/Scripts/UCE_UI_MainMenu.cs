...
public class UCE_UI_MainMenu : MonoBehaviour
{
    ...
    private NetworkAuthenticatorMMO auth; 
    ...
    private void Update()
    {
        Player player = Player.localPlayer;                                                            //Grabs the player name based on DaggerUtils.
    ...
    private IEnumerator OnLogout()
    {
        ...
        if (remainingLogoutTime <= 0)
        {
            ...
            CancelInvoke();
            network.StopClient();
            network.OnClientDisconnect();                                      //Checks to make sure the player has disconnected.
