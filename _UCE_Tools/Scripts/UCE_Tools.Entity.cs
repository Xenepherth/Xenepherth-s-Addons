...
public partial class Entity
{
    ...
    public void DealDamageAt_UCE(Entity entity, int amount, float stunChance = 0, float stunTime = 0)
    {
    ...
    {
        if (fChance <= 0 || skills.skills.Count <= 0 || !isAlive || !UCE_CanAttack(source)) return;
    ...
    {
        if (item == null) return true;
        return ((Monster)this).inventory.Count(new Item(item)) >= 1;
    }
    ...
    public bool UCE_checkHasItems(UCE_ItemRequirement[] items, bool requiresAll = false)
    {
        ...
        {
            if (((Monster)this).inventory.Count(new Item(item.item)) >= item.amount)
            {
                valid = true;
                if (!requiresAll) return valid;
            }
    ...
    {
        int totalCount = 0;
        for (int i = 0; i < ((Monster)this).inventory.slots.Count; ++i)
        {
            if (((Monster)this).inventory.slots[i].amount > 0 && ((Monster)this).inventory.slots[i].item.data == itemT)
            {
                totalCount += ((Monster)this).inventory.slots[i].amount;
            }
        }
        return totalCount;
    }
