...
public partial class UCE_LootAll : MonoBehaviour
{
    ...
    public void LootAll()
    {
        ...
        {
            if (loot.content.childCount > 0 && items.Count > 0)
            {
                ...
                lootAllBtn.interactable = player.inventory.CanAdd(items[i].item, items[i].amount);

                lootAllBtn.onClick.RemoveAllListeners();
                lootAllBtn.onClick.SetListener(() => { player.looting.TakeAllLootItem(); });
            }
        }
    }
}
